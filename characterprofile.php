...
<h3 class="panel-title"> Character Info</h3></div>
		...
		<table class="deathlist">
			<thead>
				<tr class="yellow">
					<th colspan="2" style="text-align:center">Death List</th>
				</tr>
			</thead>
			<tbody>
				<?php
				if ($config['ServerEngine'] == 'TFS_10') {
					...
					if ($deaths) {
						foreach ($deaths as $d) {
							...
							?>
							<tr>
								<td style="text-align:center"><?php echo getClock($d['time'], true, true); ?></td>
								<td>
									<?php
						...
						?>
						<tr>
							<td colspan="2" style="text-align:center">This player has never died.</td>
						</tr>
						<?php
					}
					...
					if ($array) {
						foreach ($array as $value):
							...
							?>
							<tr>
								<td style="text-align:center"><?php echo getClock($value['time'], true, true); ?></td>
								<td style="text-align:center"><?php echo 'Killed at level '. $value['level'] .' by '. $value['killed_by']; ?></td>
							</tr>
						<?php endforeach;
					} else {
						?>
					...
					if ($array) {
						...
						foreach ($array as $value):
							...
							?>
							<tr>
								<td style="text-align:center"><?php echo getClock($value['date'], true, true); ?></td>
								<td style="text-align:center"><?php echo 'Killed at level '. $value['level'] .' by '. $value[3]; ?></td>
							</tr>
						<?php endforeach;
					} else {
						?>
		...
		<center><p class="address">Address: <a href="<?php echo ($config['htwrite']) ? "//" . $_SERVER['HTTP_HOST']."/" . $profile_data['name'] : "//" . $_SERVER['HTTP_HOST'] . "/characterprofile.php?name=" . $profile_data['name']; ?>"><?php echo ($config['htwrite']) ? $_SERVER['HTTP_HOST']."/". $profile_data['name'] : $_SERVER['HTTP_HOST']."/characterprofile.php?name=". $profile_data['name']; ?></a></p>
	</center>	</div></div>
		<?php
	} else {
