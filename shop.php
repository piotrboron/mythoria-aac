...
<h3 class="panel-title"> Store</h3></div><center>
<?php
if ($loggedin === true) {
	if (!empty($_POST['buy']) && $_SESSION['shop_session'] == $_POST['session']) {
	...
	if ($config['shop_auction']['characterAuction']) {
		?>
		<br>Interested in buying characters? View the <a href="auctionChar.php">character auction page!</a><br></br>
		<?php
	}
} else {
			...
			<tr class="special">
				...
				<td>
					<form action="" method="POST">
						...
						<input type="submit" value="  PURCHASE  "  class="needconfirmation btn btn-primary btn-shiny btn-block" data-item-name="<?php echo $offers['description']; ?>" data-item-cost="<?php echo $offers['points']; ?>">
					</form>
				</td>
				<?php endif; ?>
			</tr>
